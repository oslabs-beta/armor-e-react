...
describe('input validation tests', () => {
  ...
  it('should set a custom min value', () => {
    const isValidUsername = validateInput(setError, 'tooShort', 'username', undefined, 10);
    expect(isValidUsername).toBe(false);
  });
  
  it('should set a custom max value', () => {
    const isValidUsername = validateInput(setError, 'tooLong', 'username', undefined, undefined, 5);
    expect(isValidUsername).toBe(false);
  });

  it('should set a custom max regular expression', () => {
    const isValidUsername = validateInput(setError, 'noNumbers7', 'username', undefined, undefined, undefined, /^[A-Za-z]$/);
    expect(isValidUsername).toBe(false);
  });
